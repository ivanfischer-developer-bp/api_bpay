...
class PrescripcionController extends ConexionSpController
{
    // ###############################################################################################################
    ...
    public function get_tipo_y_categoria_practicas(Request $request)
    {
        ...
        $extras = [
            ...
            'url' => '/int/recetas/tipo-y-categoria-practicas',
    ...
    /**
     * Obtiene un listado de practicas de https://apirecipe.hml.qbitos.com/apirecipe/GetPracticas/
     */
    ...
    public function generar_prescripcion_practicas(Request $request)
    {
        ...
        $extras = [
            'api_software_version' => config('site.software_version'),
            'ambiente' => config('site.ambiente'),
            'url' => '/int/recetas/generar-prescripcion-practicas',
            'controller' => explode('\\', __CLASS__)[sizeof(explode('\\', __CLASS__))-1],
            'function' => __FUNCTION__,
            'sps' => [],
            'responses' => [],
    ...
    public function listar_prescripciones_emitidas(Request $request)
    {
        ...
        $extras = [
            'api_software_version' => config('site.software_version'),
            'ambiente' => config('site.ambiente'),
            'url' => '/int/recetas/listar-prescripciones-emitidas',
            'controller' => explode('\\', __CLASS__)[sizeof(explode('\\', __CLASS__))-1],
            'function' => __FUNCTION__,
            'queries' => [],
            'responses' => [],
        ...
        try {
            ...
            // get_practicas con qbi2recipe 
            // $numero_pagina = request('numeroPagina') != null ? request('numeroPagina') : 1;
            // $params = [
            //     'practica' => eliminar_acentos(request('practica')),
            //     'tipo' => eliminar_acentos(request('tipo')),
            //     'categoria' => eliminar_acentos(request('categoria')),
            //     'numeroPagina' => request('numeroPagina'),
            // ];
            // $url = 'https://apirecipe.hml.qbitos.com/apirecipe/GetPractica/'.$param.'?clienteAppId='.env('QBI2RECIPE_IDFINANCIADOR').'&numeroPagina='.$numero_pagina;
            // $url = env('QBI2RECIPE_URLBASE').'/GetPracticas?clienteAppId='.env('QBI2RECIPE_CLIENTEAPPID').'&numeroPagina='.$numero_pagina.'&search='.reemplazar_espacios($params['practica'], '%20').'&categoria='.reemplazar_espacios($params['categoria'], '%20').'&tipo='.reemplazar_espacios($params['tipo'], '%20');
            // $extras['url_consulta'] = $url;
            // $ch = curl_init();
            ...
            //     $code = -2;
            // }else{
            //     if(sizeof($resp->practicas) == 0){
            //         $status = 'empty';
            //         $count = sizeof($resp->practicas);
            //         $message = 'No se encontraron resultados conincidentes con los valores buscados.';
            //         $data = $resp;
            //         $code = 0;
            //     }else{
            ...
            //         //     // return response()->json([
            //         //     //     'busqueda' => $param,
            //         //     //     'count_medicamentos' => sizeof($resp->medicamentos),
            //         //     //     'count_vademecum' => sizeof($vademecum),
            //         //     //     'medicamentos' => $resp->medicamentos,
            //         //     //     'vademecum' => $vademecum
            //         //     // ]);
            ...
            //         //                 if($resp->medicamentos[$i]->nombreProducto == $vademecum[$j]->nombre){
